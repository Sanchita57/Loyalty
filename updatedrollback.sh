...
# Define the backup directory and its parent
backup_dir="/tmp/backup/${current_date}/SC"
to_delete_dir="/tmp/backup/${current_date}"
echo "Backup directory: ${backup_dir}"
echo "To delete directory: ${to_delete_dir}"
...
if [ ! -d "${backup_dir}" ]; then
  ...
  if [ -n "${latest_backup_dir}" ] && [ -d "${latest_backup_dir}" ] && [ "$(ls -A "${latest_backup_dir}")" ]; then
    echo "Using latest backup directory: ${latest_backup_dir}"
    source_dir="${latest_backup_dir}"
    to_delete_dir=$(dirname "${latest_backup_dir}")
  else
    echo "No valid backup directories found. Rollback failed."
    exit 1
  fi
...
if [ $? -eq 0 ]; then
  ...
  # Delete the entire date folder used for rollback
  echo "Deleting backup date folder used for rollback: ${to_delete_dir}..."
  rm -rf "${to_delete_dir}"
  if [ $? -eq 0 ]; then
    echo "Backup date folder ${to_delete_dir} has been deleted."
  else
    echo "Failed to delete backup date folder ${to_delete_dir}. Please check permissions."
  fi
else
  echo "Rollback not done. Copy command failed."
fi
